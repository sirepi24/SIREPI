...
if genre == '***AGUA***':
    ...
    sql1 = 'SELECT ANY_VALUE(PROVINCIA) as PROVINCIA, SUM("DenGest1eraATC") AS GESTANTES, SUM("NumGest1eraATC_1erTri") AS GESTANTES1TRI,(SUM("NumGest1eraATC_1erTri")*100)/SUM("DenGest1eraATC") AS PORCENTAJE  FROM HIS GROUP BY PROVINCIA;'
    total_orden = conn1.query(sql=sql1, spreadsheet=url1)
    st.dataframe(total_orden)

    st.subheader("SEGUNDO INDICADOR: Porcentaje de niños y niñas de 4 meses")
    sql2 = 'SELECT ANY_VALUE(PROVINCIA) as PROVINCIA, SUM("Den4mes") AS Den4mes, SUM("Num4mes") AS Num4mes, (SUM("Num4mes")*100)/SUM("Den4mes") AS PORCENTAJE FROM HIS GROUP BY PROVINCIA;'
    ...
    sql3 = 'SELECT ANY_VALUE(PROVINCIA) as PROVINCIA, SUM("DenCREDmes") AS DenCREDmes, SUM("NumCREDmes") AS NumCREDmes, (SUM("NumCREDmes")*100)/SUM("DenCREDmes") AS PORCENTAJE FROM HIS GROUP BY PROVINCIA;'
